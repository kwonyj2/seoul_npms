...
class NetworkTopologyViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def _parse_nas_rack(self, school_name):
        """NAS 랙실장도 엑셀 파싱 — 다중 랙 자동 감지 (6열 간격)"""
        import os
        ...
        # 멀티시트: 학교명 시트 또는 랙실장도 시트 우선
        ws = None
        for sname in wb.sheetnames:
            if school_name in sname or '랙' in sname or '렉' in sname:
                ws = wb[sname]
                break
        if ws is None:
            # 학교명/포트맵이 아닌 첫 시트
        ...
        # 1단계: 통신랙 이름이 있는 열(col) 수집 → 랙 위치 자동 감지
        rack_positions = []  # [(u_col, eq_col, rack_name, location)]
        rack_name_cols = {}
        for r in range(1, 8):
        ...
        # 2단계: 각 랙 파싱
        racks = []
        for rack_idx, (u_col, eq_col, rack_name, location) in enumerate(rack_positions):
            # U=1 시작행 찾기
            u1_row = None
            for r in range(3, 15):
                v = ws.cell(r, u_col).value
...
.port-table tbody td {{ border-bottom: 0.5px solid #eceff1; font-size: 6.5pt; height: 5mm; line-height: 5mm; padding: 0 3px; }}
    ...
    def pptx_extract_run(self, request):
        """구성도 PPTX 일괄 추출 → 엑셀 파일 생성 (pptx_extractor 활용)"""
        import io, re as _re
        from openpyxl import Workbook
        from openpyxl.styles import Alignment, Border, Font, PatternFill, Side
        from .pptx_extractor import extract_pptx_network
        ...
        for fname in files:
            ...
            try:
                ...
                school = rows[0]['학교명'] if rows else _extract_school_name(fname)
        ...
        all_rows.sort(key=lambda r: (r['학교명'], r.get('슬라이드', 0),
                                     TIER_ORDER.get(r.get('계위', ''), 9),
                                     NET_ORDER.get(r['망'], 9), r['망ID']))
        ...
        sh = ['학교명', '총 장비수', '교사망', '학생망', '무선망', '기타망', '보안',
              '1계위', '2계위', '3계위', '포트정보 보유', '케이블 추출']
        ...
        for ri, (school, srows) in enumerate(sorted(schools.items()), start=2):
            ...
            tiers = [r.get('계위', '') for r in srows]
            vals = [school, len(srows), nets.count('교사망'), nets.count('학생망'),
                    nets.count('무선망'), nets.count('기타망'), nets.count('보안(공통)'),
                    tiers.count('1계위'), tiers.count('2계위'), tiers.count('3계위'),
                    sum(1 for r in srows if r.get('포트(다운→업)')),
                    sum(1 for r in srows if r.get('케이블 출처') == '추출')]
            for ci, v in enumerate(vals, 1):
